 ...
 * current-streak ring highlighted in the theme accent. Self-contained SVG,
 * all user-derived text escaped.
 */
export function renderStreakCard(
  ...
  const dividers = `
  <line x1="${COL}" y1="35" x2="${COL}" y2="${HEIGHT - 35}" stroke="${theme.borderColor}"/>
  <line x1="${COL * 2}" y1="35" x2="${COL * 2}" y2="${HEIGHT - 35}" stroke="${theme.borderColor}"/>`;
...
function sideColumn(
  ...
  return `<g text-anchor="middle" font-family="${FONT_STACK}">
    <text x="${cx}" y="78" font-size="30" font-weight="800" fill="${theme.textColor}">${escapeXml(value)}</text>
    <text x="${cx}" y="108" font-size="14" font-weight="700" fill="${theme.titleColor}">${escapeXml(label)}</text>
    <text x="${cx}" y="132" font-size="11" fill="${theme.mutedColor}">${escapeXml(caption)}</text>
  </g>`;
}
...
function currentColumn(cx: number, streak: StreakRange, theme: Theme): string {
  ...
  return `<g font-family="${FONT_STACK}" text-anchor="middle">
    <circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${theme.accentColor}" stroke-width="4"/>
    <text x="${cx}" y="${cy + 9}" font-size="28" font-weight="800" fill="${theme.accentColor}">${streak.length}</text>
    <text x="${cx}" y="140" font-size="14" font-weight="700" fill="${theme.titleColor}">Current Streak</text>
    <text x="${cx}" y="162" font-size="11" fill="${theme.mutedColor}">${escapeXml(streakCaption(streak))}</text>
  </g>`;
}
